...
if [[ "$1" == "non-initial" ]]; then
  ...
  PROMPT_DATE=$(TZ='UTC' date "+%Y-%m-%d %H:%M:%S")

  # blocking call
  RES=$(notify-send.py -u critical -i media-playback-pause -a "Breaky" "Take a break" "Take a short break, stretch, relax your eyes by looking into the distance" --action break_full:"Full Break" break_short:"Short Break" skip:Skip | head -1)

  CLICK_DATE=$(TZ='UTC' date "+%Y-%m-%d %H:%M:%S")
